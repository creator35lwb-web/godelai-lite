...
**Version:** 1.3 (Session 4 — Verification Unblocked + Kaggle Pipeline Restored)  
**Lineage:** Rk-genesis-prompt-v1.0.md → v1.1.md → v1.2.md → v1.3.md  
...
**MACP Handoff:**
- Session 4 handoff: `.macp/handoff-rk-session-004.md`
...
HF DOWNLOAD (fallback only — subject to rate limits during hackathon load):
  ...
  Model ID casing: google/gemma-4-E4B-it (capital E4B) — lowercase 404s
...
QUANTIZATION (GPU T4 only, not TPU):
...
| v2.1 | bitsandbytes 4-bit quant, corrected model ID | GPU ran 490s+ |
| v2.2 | Removed token requirement, capital E4B fix, shared model weights | Ran 300s+ |
| v2.3 | Offline Kaggle model source + robust bitsandbytes | Pushed |
| v2.4 | TPU v5e-8 support (accelerator detection, bfloat16, xm.mark_step) | ERROR — CPU node |
| **v2.5** | **Kaggle path detection first, HF fallback, model_sources in metadata** | **QUEUED (canonical)** |

### 5.6 Remaining Competition Tasks

| Priority | Task | Blocked By |
|----------|------|------------|
| 1 | v2.5 successful run with benchmark output | v2.5 currently QUEUED |
| 2 | Download benchmark output (write to /kaggle/working/ or check UI) | Step 1 |
| 3 | Write `GODELAI-Lite-Writeup.md` with actual numbers | Step 2 |
| 4 | Final notebook polish — writeup narrative as markdown cells | Step 3 |
| 5 | Submit final notebook as competition entry | Alton confirmation |
...
| **Quantization** | bitsandbytes 4-bit (GPU only — not TPU) |
